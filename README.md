...
```
После удаления PV файл shared.txt останется на локальном диске,
поскольку он создается в локальной файловой системе, а не в пространстве, 
управляемом Kubernetes. Это объясняется тем, что hostPath монтирует 
директорию непосредственно на ноду, и файлы, созданные в этой директории, 
сохраняются даже после удаления PV.
```
------
